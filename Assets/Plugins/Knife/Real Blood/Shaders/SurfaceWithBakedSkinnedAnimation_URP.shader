...
Shader "URP/Custom/SurfaceWithBakedSkinnedAnimation_URP_Fixed"
{
    Properties
    {
        ...
        [HideInInspector]_LocalPosition("Instance Pos", Vector) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                OUT.normalWS = normalize(mul((float3x3)GetObjectToWorldMatrix(), animatedNormal));
                OUT.uv = IN.uv;
            ...
            {
                UNITY_SETUP_INSTANCE_ID(IN);
                float3 albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, IN.uv).rgb * _Color.rgb * UNITY_ACCESS_INSTANCED_PROP(Props, _InstancedColor).rgb;
